...
def UploadFile(a, filesname, f):
    url = (
        r"https://graph.microsoft.com/v1.0/me/drive/root:/AutoApi/App"
        + str(a)
        + r"/"
        + filesname
        + r":/content"
...
def taskWrite(a, taskname):
    ...
    url = (
        r"https://graph.microsoft.com/v1.0/me/todo/lists/" + listjson["id"] + r"/tasks"
    )
    data = {
        "title": taskname,
    }
...
def teamWrite(a, channelname):
    ...
    url = (
        r"https://graph.microsoft.com/v1.0/teams/" + objectlist[0]["id"] + r"/channels"
    )
    jsontxt = json.loads(apiReq("post", a, url, json.dumps(data)))
    url = (
        r"https://graph.microsoft.com/v1.0/teams/"
        + objectlist[0]["id"]
        + r"/channels/"
        + jsontxt["id"]
    )
